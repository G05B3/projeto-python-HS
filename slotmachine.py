...
# Constantes
# número de símbolos (o programa funciona para qualquer número de símbolos > 1)
SYMB_NUM = 3
...
class SlotMachine():
    ...
    def roll(self):
        for i in range(SYMB_NUM):
            ...
            while (aux > 0):
                j += 1
                # subtrai as chances para descobrir a gama de valores em que se encontra
                aux -= CHANCES[j]
    ...
    def display(self):
        ...
        for i in range(29):
            sys.stdout.write('\r'+"[ ")
            for k in range(SYMB_NUM):
                # símbolo a desenhar nesta iteração
                iter_sym = SYMBOLS.index(self.val[k])+anim[k]
                print(color(iter_sym) +
                      SYMBOLS[iter_sym]+Style.RESET_ALL+" ", end="")
                if k < SYMB_NUM-1:
                    print("| ", end="")
            print("]", end="")
            sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    while P.credits > 0 and isRunning == True:
        ...
        print(f"You have {P.credits} credits at this moment."
